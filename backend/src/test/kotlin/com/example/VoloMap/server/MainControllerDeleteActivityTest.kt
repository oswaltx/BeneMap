...
class MainControllerDeleteActivityTest {
    ...
    fun `owner can delete their activity, ratings are removed first`() {
        ...
        val controller = MainController(repository, geocodingService, userRepository, activityRatingRepository, mock(), mock(), RateLimiter(enabled = true), mock())
    ...
    fun `owner can delete their activity, signups are removed first`() {
        ...
        val controller = MainController(repository, geocodingService, userRepository, mock(), mock(), activitySignupRepository, RateLimiter(enabled = true), mock())
        val result = controller.deleteActivity(5, authenticationFor(owner.email))
